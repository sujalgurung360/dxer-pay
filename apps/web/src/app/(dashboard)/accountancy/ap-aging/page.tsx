...
export default function ApAgingPage() {
  ...
  const loadData = async () => {
    ...
    setError(null);
    try {
      const res = (await api.accountancy.apAging(asOf)) as AgingResponse;
      setData(res.data);
    } catch (err: any) {
      setError(err.message || 'Failed to load AP aging');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div>
      <PageHeader
        ...
        description="Outstanding payables approximated from expenses by category and aging bucket"
      />
      ...
      <div className="card">
        <h2 className="mb-2 text-sm font-semibold text-gray-900">Aging by category</h2>
        {!data ? (
          <p className="text-xs text-gray-400">{loading ? 'Loading…' : 'No expenses found.'}</p>
        ) : (
